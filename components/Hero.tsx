...
const Hero = () => {
  ...
  const [latestVideo, setLatestVideo] = useState<string>(
    "https://youtube.com/@atadogann"
  );
      ...
      <div className="absolute inset-0 z-0">
        <img
          ...
          className={`w-full h-full object-cover transition-all duration-1000 ${
            loading ? "blur-md scale-105" : "blur-sm opacity-40 scale-100"
          }`}
        />
        ...
        <div className="space-y-6 text-center md:text-left">
            ...
            {status?.isLive ? (
              <div className="glass-strong p-5 rounded-xl animate-in fade-in zoom-in duration-500 glow-purple border border-purple-500/30">
                <p className="font-bold text-white flex items-center gap-2 mb-2">
                  ...
                  Şuan Oynanıyor:{" "}
                  <span className="gradient-text">{status.game}</span>
                </p>
                <p className="text-sm text-zinc-300 line-clamp-1 italic opacity-80">
                  "{status.title}"
                </p>
                <div className="mt-3 flex items-center gap-2 text-xs font-bold text-pink-500 bg-pink-500/10 px-3 py-1.5 rounded-lg w-fit border border-pink-500/20">
                  ...
                  {status.viewerCount.toLocaleString()} İzleyici
                </div>
              </div>
            ) : (
              <>
                ...
                <span className="opacity-75">
                  Oyun, Trakya samimiyeti ve bitmeyen goygoy için doğru
                  yerdesin.
                </span>
              </>
            )}
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center md:justify-start gap-4 pt-4">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="group glass-strong px-8 py-4 rounded-xl font-bold hover:glow-purple transition-all cursor-pointer flex items-center gap-3 border border-purple-500/30 hover:border-purple-500"
            >
              <Youtube className="w-5 h-5 text-purple-500" />
              <span className="gradient-text">Son Videoyu İzle</span>
            </a>
            <a
              href="https://kick.com/atadogann"
              target="_blank"
              rel="noreferrer"
              className="group relative px-8 py-4 rounded-xl font-bold transition-all flex items-center gap-3 overflow-hidden bg-gradient-to-r from-purple-600 to-pink-600 hover:shadow-2xl hover:shadow-purple-500/50"
            >
              <KickIcon className="w-5 h-5 relative z-10" />
              <span className="relative z-10">
                {status?.isLive ? "Yayına Katıl" : "Takip Et"}
              </span>
        ...
        <div className="relative hidden md:block h-[600px] w-full">
          {loading ? (
            ...
            <>
              ...
              <img
                src="https://pbs.twimg.com/media/GtPkeP5WMAAZgzl.jpg"
                alt="Ataberk Doğan"
